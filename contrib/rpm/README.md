...
The RPM spec file provided here is for Skyrus-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://skyrus.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Skyrus git repository](https://github.com/skyrus/skyrus) but are not part of
...
Currently only `Source10` is used, for the example `skyrus.conf` file.
...
in the skyrus git repository. At present only an SVG version of the Skyrus
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the skyrus
...
needed when 0.12.1 is released, a proper fix is already in the Skyrus git
master branch.
...
idea to build Skyrus Core against a static version of that library compiled
...
The `skyrus.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `skyrus-server` package. At this point, most RPM based
...
The `skyrus-server` package should have SELinux support. How to properly do
...
Skyrus-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/skyrus/). He has been
packaging Skyrus for Fedora at least since 2012.
...
4. I renamed the `skyrus` package that contains the Qt GUI to `skyrus-core` as
that appears to be how the general population refers to it, in contrast to
`skyrus-xt` or `skyrus-classic`. I wanted to make sure the general population
