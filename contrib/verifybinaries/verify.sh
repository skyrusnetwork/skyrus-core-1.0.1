...
###   This script attempts to download the signature file SHA256SUMS.asc from skyrus.org
...
WORKINGDIR="/tmp/skyrus"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://skyrus.org/bin/"
VERSIONPREFIX="skyrus-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'skyrus-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Skyrus Core binary release signing key installed?"
   fi
