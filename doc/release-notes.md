Skyrus Core version 0.13.0 is now available from:

  <https://skyrus.org/bin/skyrus-core-0.13.0/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/skyrus/skyrus/issues>

To receive security and update notifications, please subscribe to:

  <https://skyruscore.org/en/list/announcements/join/>
...
updates will be released anymore. Without security updates, using a skyrus
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Skyrus Core
randomly crashing on Windows XP. It is [not clear](https://github.com/skyrus/skyrus/issues/7681#issuecomment-217439891)
...
- Adding `dbcache=100` in skyrus.conf
...
For example:
    ...
    $ src/skyrus-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Skyrus Core code base has
...
- `skyrus-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `skyrus-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
compatible peers.
[BIP 152](https://github.com/skyrus/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Skyrus Core.

[Pull request](https://github.com/skyrus/skyrus/pull/8035/files), [BIP 32](https://github.com/skyrus/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/skyrus/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/skyrus/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/skyrus/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/skyrus/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of Skyrus Core is expected that
...
Witness)] (https://github.com/skyrus/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/skyrus/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Skyrus-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/skyrus/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/skyrus/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/skyrus/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/skyrus/skyrus/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for skyrus-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use SKX/kB (MarcoFalke)
...
- #7692 `29b2be6` Remove P2P alert system (SKXdrak)
...
- #8275 `042c323` Remove bad chain alert partition check (SKXdrak)
...
- #7619 `18b3f1b` Add missing sudo entry in gitian VM setup (SKXdrak)
- #7616 `639ec58`  [depends] Delete unused patches  (MarcoFalke)
- #7658 `c15eb28` Add curl to Gitian setup instructions (SKXdrak)
...
- #7613 `3798e5d` Add autocomplete to skyrus-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add skyrus address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: skyrus-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from skyrusd (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Skyrus-Core repository and website to the About Dialog (MarcoFalke)
...
- #7531 `18cb2d5` Add bip68-sequence.py to extended rpc tests (SKXdrak)
...
- #7773 `7c80e72` Fix comments in tests (SKXdrak)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for skyrusd procs (MarcoFalke)
...
- #8164 `0f24eaf` [Skyrus-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of skyrusd startup (laanwj)
...
- #7975 `fc23fee` Update skyrus-core GitHub links (MarcoFalke)
...
- #7540 `ff46dd4` Rename OP_NOP3 to OP_CHECKSEQUENCEVERIFY (SKXdrak)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/skyrus/).
