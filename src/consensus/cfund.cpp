...
bool CFund::IsValidPaymentRequest(CTransaction tx)
{
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nAmount) + "SKX from the proposal " + proposal.hash.ToString() + ". Payment request id: " + strDZeel;
...
bool CFund::IsValidProposal(CTransaction tx)
{
    ...
    CSkyrusAddress address(Address);
    if (!address.IsValid())
        return false;
