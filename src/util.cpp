...
const char * const SKYRUS_CONF_FILENAME = "skyrus.conf";
const char * const SKYRUS_PID_FILENAME = "skyrus.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Skyrus
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Skyrus
    // Mac: ~/Library/Application Support/Skyrus
    // Unix: ~/.skyrus
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Skyrus4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Skyrus4";
#else
    // Unix
    return pathRet / ".skyrus4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", SKYRUS_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No skyrus.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override skyrus.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", SKYRUS_PID_FILENAME));
