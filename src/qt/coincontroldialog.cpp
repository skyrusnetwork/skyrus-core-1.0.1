    ...
    platformStyle(platformStyle)
{
    ...
    // see https://github.com/skyrus/skyrus/issues/5716
...
{
    GUIUtil::setClipboard(SkyrusUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = SkyrusUnits::SKX;
    ...
    l2->setText(SkyrusUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(SkyrusUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(SkyrusUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(SkyrusUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(SkyrusUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000))) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(SkyrusUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CSkyrusAddress(outputAddress).ToString());

                // if listMode or change => show skyrus address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, SkyrusUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, SkyrusUnits::format(nDisplayUnit, nSum));
