...
const std::string SkyrusGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SkyrusGUI::DEFAULT_WALLET = "~Default";

SkyrusGUI::SkyrusGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void SkyrusGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/skyrus"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Skyrus address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and skyrus: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Skyrus addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Skyrus addresses"));
    ...
    openAction->setStatusTip(tr("Open a skyrus: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Skyrus command-line options").arg(tr(PACKAGE_NAME)));
...
void SkyrusGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(SkyrusUnits::Unit u, SkyrusUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(SkyrusUnits::name(u)), this);
...
void SkyrusGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void SkyrusGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        topMenuLogo->setObjectName("skxLogo");
        topMenuLogo->move(0,0);
        topMenuLogo->setStyleSheet(
           "#skxLogo { border-image: url(:/icons/menu_logo)  0 0 0 0 stretch stretch; border: 0px; }");
        ...
        // ImageButton* skxLogo2 = new ImageButton();
        // skxLogo2->setFixedSize(64,64);
        // skxLogo2->setObjectName("skxLogo");
        // skxLogo2->setPixmap(pixmap);
        //
        // toolbar->setMovable(false);
        // toolbar->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        // toolbar->addWidget(skxLogo);
        // toolbar->addWidget(skxLogo2);
...
void SkyrusGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool SkyrusGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void SkyrusGUI::startVotingCounter()
{
    if (GetBoolArg("-staking", true))
    {
...
bool SkyrusGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SkyrusGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SkyrusGUI::setWalletActionsEnabled(bool enabled)
{
...
void SkyrusGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void SkyrusGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SkyrusGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SkyrusGUI::showDebugWindow()
{
...
void SkyrusGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SkyrusGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void SkyrusGUI::openClicked()
{
...
void SkyrusGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void SkyrusGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SkyrusGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SkyrusGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SkyrusGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SkyrusGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SkyrusGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void SkyrusGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Skyrus network", "", count));
    if(walletFrame){
...
void SkyrusGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to Skyrus network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to Skyrus network..."));
...
void SkyrusGUI::showVotingDialog()
{
...
void SkyrusGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Skyrus"); // default title
...
void SkyrusGUI::changeEvent(QEvent *e)
{
...
void SkyrusGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SkyrusGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SkyrusGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SkyrusUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void SkyrusGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SkyrusGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SkyrusGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SkyrusGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SkyrusGUI::setEncryptionStatus(int status)
{
...
void SkyrusGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SkyrusGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SkyrusGUI::detectShutdown()
{
...
void SkyrusGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SkyrusGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(SkyrusGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SkyrusGUI::subscribeToCoreSignals()
{
...
void SkyrusGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<SkyrusUnits::Unit> units = SkyrusUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const SkyrusUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(SkyrusUnits::name(unit)));
    }
...
{
    setText(SkyrusUnits::name(newUnits));
}
...
void SkyrusGUI::updatePrice()
{
  ...
  request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/skx-coin/?convert=EUR"));
...
void SkyrusGUI::replyFinished(QNetworkReply *reply)
{
...
void SkyrusGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CSkyrusAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void SkyrusGUI::getVotingInfo()
{
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.skyrus.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void SkyrusGUI::updateStakingStatus()
{
    updateWeight();
