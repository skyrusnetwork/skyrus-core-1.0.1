...
const int SKYRUS_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString SKYRUS_IPC_PREFIX("skyrus:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
const char* NIP01_MESSAGE_SIGNEDMSG = "SignedMsg";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/skyrus-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/skyrus-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/skyrus-paymentrequest";
const char* NIP01_MIMETYPE_SIGNEDMSG = "application/skyrus-signedmsg";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the skyrus: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(SKYRUS_IPC_PREFIX, Qt::CaseInsensitive)) // skyrus: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseSkyrusURI(arg, &r) && !r.address.isEmpty())
            {
                CSkyrusAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(SKYRUS_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(SKYRUS_IPC_CONNECT_TIMEOUT);
    ...
    model(0)
{
    ...
    // on Mac: sent when you click skyrus: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            LogPrintf("Cannot start skyrus: click-to-pay handler\n");
        }
        else {
...
//
// OSX-specific way of handling skyrus: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in skyrus: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(SKYRUS_IPC_PREFIX, Qt::CaseInsensitive)) // skyrus: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        if (uri.hasQueryItem("m") && uri.hasQueryItem("a")) // sign message URI
        {
            ...
            CSkyrusAddress addr(uri.queryItemValue("a").toStdString());
            if (!addr.IsValid())
            {
            ...
            QUrl fetchUrlConstructed(s.mid(SKYRUS_IPC_PREFIX.length()));
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseSkyrusURI(s, &recipient))
            {
              ...
              if (!address.IsValid()) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Skyrus address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CSkyrusAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom skyrus addresses are not supported
        ...
        // Skyrus amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(SkyrusUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
