...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key)
{
    ...
    if(hasPayment){
        ...
        CSkyrusAddress address2(address1);
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CSkyrusAddress(CScriptID(redeemScript)).ToString();
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl* coinControl, bool sign, std::string strDZeel)
{
    ...
    txNew.strDZeel = (wtxNew.strDZeel != "" ? wtxNew.strDZeel : strDZeel).length() > 0 ?
                ...
                skxtech.EncryptAddress(std::to_string(GetAdjustedTime() + (rand() % 1<<8)),sPubKey);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-skyrus-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CSkyrusAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CSkyrusAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CSkyrusAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CSkyrusAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CSkyrusAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CSkyrusAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CSkyrusAddress(dest).ToString(), key);
}
