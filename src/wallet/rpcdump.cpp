...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"skyrusprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"skyrusprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CSkyrusAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CSkyrusAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Skyrus address or script");
    }
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CSkyrusAddress(pubKey.GetID()), strLabel);
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"skyrusaddress\"\n"
            "\nReveals the private key corresponding to 'skyrusaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"skyrusaddress\"   (string, required) The skyrus address for the private key\n"
            "\nResult:\n"
    ...
    return CSkyrusSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Skyrus %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CSkyrusAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CSkyrusSecret(key).ToString(), strTime);
