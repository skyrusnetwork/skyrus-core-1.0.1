...
UniValue Skxtech::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    UniValue addresses = find_value(serverData, "skx_addresses");
    UniValue addrArray = addresses.get_array();

    UniValue skxtechData;
    skxtechData.setObject();
    skxtechData.pushKV("anondestination", encryptedAddress);
    skxtechData.pushKV("public_key", pubKey);
    skxtechData.pushKV("anonaddress", addrArray);
    skxtechData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    skxtechData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return skxtechData;
  } else {
    throw runtime_error("Unable to send SKXTech transaction, please try again.");
  }

}

vector<anonServer> Skxtech::GetAnonServers() {

  vector<anonServer> returnServers;

  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one SKXTech server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid SKXTech server to send private payments.");
      return returnServers;
  }
...
UniValue Skxtech::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured SKXTech nodes are available right now.");
  }
...
UniValue Skxtech::ParseJSONResponse(string readBuffer) {
  try {
      UniValue valRequest;
...
string Skxtech::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {

  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",SKXTECH_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
...
int Skxtech::PublicEncrypt(unsigned char* data, int data_len, unsigned char* key, unsigned char* encrypted)
{
...
RSA * Skxtech::CreateRSA(unsigned char * key,int isPublic)
{
    RSA *rsa= NULL;
    BIO *keybio ;
...
bool Skxtech::TestEncryption(string encrypted, UniValue serverData) {
...
UniValue Skxtech::GetServerInfo(std::string server) {
  curl_global_init(CURL_GLOBAL_ALL);
  curl = curl_easy_init();
  string readBuffer;
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue skxtechData;
        skxtechData.setObject();
        skxtechData.pushKV("min_amount", find_value(data, "min_amount"));
        skxtechData.pushKV("max_amount", find_value(data, "max_amount"));
        skxtechData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return skxtechData;
      }
