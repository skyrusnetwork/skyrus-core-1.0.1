...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new Skyrus address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"skyrusaddress\"    (string) The new skyrus address\n"
            "\nExamples:\n"
    ...
    return CSkyrusAddress(keyID).ToString();
}


CSkyrusAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CSkyrusAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Skyrus address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"skyrusaddress\"   (string) The account skyrus address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Skyrus address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"skyrusaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"skyrusaddress\"  (string, required) The skyrus address to be associated with an account.\n"
    ...
    CSkyrusAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Skyrus address");
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"skyrusaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"skyrusaddress\"  (string, required) The skyrus address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"skyrusaddress\"  (string) a skyrus address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    BOOST_FOREACH(const PAIRTYPE(CSkyrusAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CSkyrusAddress& address = item.first;
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw runtime_error(
            "sendtoaddress \"skyrusaddress\" amount ( \"comment\" \"comment-to\" \"anon-destination\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"skyrusaddress\"  (string, required) The skyrus address to send to.\n"
            ...
            "5. \"anon-destination\"  (string, optional) Encrypted destination address if you're sending a SKXtech transaction \n"
            "6. subtractfeefromamount  (boolean, optional, default=false) The fee will be deducted from the amount being sent.\n"
            "                             The recipient will receive less skyruss than you enter in the amount field.\n"
            "\nResult:\n"
    ...
    CSkyrusAddress address(address_str);
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Skyrus address");
...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 4)
        throw runtime_error(
            ...
            "\nCreates a proposal for the community fund. Min fee of " + std::to_string((float)Params().GetConsensus().nProposalMinimalFee/COIN) + "SKX is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"skyrusaddress\"     (string, required) The skyrus address where coins would be sent if proposal is approved.\n"
    ...
    CSkyrusAddress address("NQFqqMUD55ZV3PJEJZtaKCsQmjLT6JkjvJ"); // Dummy address
    ...
    CSkyrusAddress destaddress(Address);
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nCreates a proposal to withdraw funds from the community fund. Fee: 0.0001 SKX\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    CSkyrusAddress address(proposal.Address);

    if(!address.IsValid())
        throw JSONRPCError(RPC_TYPE_ERROR, "Address of the proposal is not a valid Skyrus address.");
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nReqAmount) + "SKX from the proposal " + proposal.hash.ToString() + ". Payment request id: " + id;
...
UniValue donatefund(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "                             The fund will receive less skyruss than you enter in the amount field.\n"
            "\nResult:\n"
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
...
UniValue anonsend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "anonsend \"skyrusaddress\" amount ( \"comment\" \"comment-to\" )\n"
    ...
    int nEntropy = GetArg("anon_entropy",SKXTECH_DEFAULT_ENTROPY);
    ...
    UniValue skxtechData = skxtech.CreateAnonTransaction(params[0].get_str(), nAmount / (nTransactions * 2), nTransactions);
    std::vector<UniValue> serverSkxAddresses(find_value(skxtechData, "anonaddress").getValues());

    if(serverSkxAddresses.size() != nTransactions)
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "SKXTech server returned a different number of addresses.");

    for(unsigned int i = 0; i < serverSkxAddresses.size(); i++)
    {
        CSkyrusAddress serverSkxAddress(serverSkxAddresses[i].get_str());
        if (!serverSkxAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Skyrus address provided by SKXTech server");
    }
    ...
    CAmount nMinAmount = find_value(skxtechData, "min_amount").get_int() * COIN;
    UniValue pubKey = find_value(skxtechData, "public_key");
    ...
    for(unsigned int i = 0; i < serverSkxAddresses.size(); i++)
    {
        ...
        if(i == serverSkxAddresses.size() - 1 || (nAmountNotProcessed - nAmountToSubstract) < (nMinAmount + 0.001))
        {
            nAmountRound = nAmountNotProcessed;
            i = serverSkxAddresses.size();
        }
        else
        {
        ...
        string encryptedAddress = skxtech.EncryptAddress(params[0].get_str(), pubKey.get_str(), serverSkxAddresses.size(), i+(i==serverSkxAddresses.size()?0:1), nId);
        wtx.strDZeel = encryptedAddress;
        SendMoney(serverSkxAddress.Get(), nAmountRound, fSubtractFeeFromAmount, wtx, encryptedAddress);
    }
...
UniValue getanondestination(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getanondestination \"skyrusaddress\"\n"
            ...
            "\"anonaddress\"  (string) The skx coin address to send the anon transaction to.\n"
            "\nExamples:\n"
    ...
    UniValue skxtechData = skxtech.CreateAnonTransaction(params[0].get_str());

    CSkyrusAddress serverSkxAddress(find_value(skxtechData, "anonaddress").get_str());
    if (!serverSkxAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Skyrus address provided by SKXTech server");
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"skyrusaddress\",     (string) The skyrus address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CSkyrusAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CSkyrusAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CSkyrusAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"skyrusaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"skyrusaddress\"  (string, required) The skyrus address to use for the private key.\n"
    ...
    CSkyrusAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"skyrusaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given skyrusaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"skyrusaddress\"  (string, required) The skyrus address for transactions.\n"
    ...
    // Skyrus address
    CSkyrusAddress address = CSkyrusAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Skyrus address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toskyrusaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a skyrus address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toskyrusaddress\"  (string, required) The skyrus address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CSkyrusAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Skyrus address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The skyrus address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less skyruss than you enter in their corresponding amount field.\n"
    ...
    {
        CSkyrusAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Skyrus address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Skyrus address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of skyrus addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) skyrus address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"skyrusaddress\"  (string) A skyrus address associated with the keys.\n"
    ...
    return CSkyrusAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    return CSkyrusAddress(w.result).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CSkyrusAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CSkyrusAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CSkyrusAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"skyrusaddress\",    (string) The skyrus address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"skyrusaddress\",    (string) The skyrus address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"skyrusaddress\",   (string) The skyrus address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending skyruss\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending skyrus\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"skyrusaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Skyrus server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending skyruss.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of skyrus addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) skyrus address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the skyrus address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CSkyrusAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Skyrus address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CSkyrusAddress(address).ToString()));
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"     (string, optional, default pool address) The skyrus address to receive the change\n"
    ...
    if (params.size() > 1) {
      if (params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CSkyrusAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid skyrus address");
...
UniValue resolveopenalias(const UniValue& params, bool fHelp)
{
  ...
  if ((fHelp || params.size() != 1))
      throw runtime_error(
          ...
          "\nResolves the give OpenAlias address to a Skyrus address.\n"
          "\nArguments:\n"
          "1. \"address\"    (string) The OpenAlias address.\n"
          "\nExamples:\n"
          "\nGet information about an OpenAlias address\n"
          + HelpExampleCli("resolveopenalias", "\"donate@skyrus.org\"")
      );
