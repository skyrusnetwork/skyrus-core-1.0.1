...
// Copyright (c) 2017-2018 The Skyrus Core developers
...
#if defined(NDEBUG)
# error "Skyrus cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Skyrus Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight == 1)
    {
        CAmount nSubsidy = (40000000 - 100 * 50) * COIN;
        return nSubsidy;
    }
    ...
    // nSubsidy >>= halvings;
    if(nHeight > 101) nSubsidy = 0;//till LastPowBlock
    return nSubsidy;
}
...
bool DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    if (!pcfundindex->UpdateProposalIndex(proposalIndex)) {
        return AbortNode(state, "Failed to write proposal index");
    }

    if (!pcfundindex->UpdatePaymentRequestIndex(paymentRequestIndex)) {
        return AbortNode(state, "Failed to write proposal index");
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck, bool fProofOfStake)
{
    ...
    if (!pcfundindex->WriteProposalIndex(proposalIndex))
        return AbortNode(state, "Failed to write proposal index");
...
//
// Commented - Skyrus uses now version control
//
...
bool CountVotes(CValidationState& state, CBlockIndex *pindexNew, const CBlock *pblock)
{
    ...
    if (!pcfundindex->UpdateProposalIndex(vecProposalsToUpdate)) {
        return AbortNode(state, "Failed to write proposal index");
    }
    if (!pcfundindex->UpdatePaymentRequestIndex(vecPaymentRequestsToUpdate)) {
        return AbortNode(state, "Failed to write payment request index");
    }
    return true;
}
        ...
        for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++) {
            ...
            if(block.vtx[0].vout[i].nValue > 0) {
                ...
                if(metadata[nPaymentRequestsCount].isStr()) {
                    ...
                    if(block.vtx[0].vout[i].nValue != prequest.nAmount || prequest.fState != CFund::ACCEPTED || proposal.Address != CSkyrusAddress(address).ToString())
                        ...
                        if(!pcfundindex->UpdatePaymentRequestIndex(vec))
                            return AbortNode(state, "Failed to write payment request index");
                    }
                } else {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    if (pfrom->nVersion != 0)
    {
        ...
        {
            reason = "You are using an old version of Skyrus, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70017 && IsWitnessEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Segregated Witness has been enabled and you are using an old version of Skyrus, please update.";
            fObsolete = true;
        }

        if(pfrom->nVersion < 70020 && IsCommunityFundEnabled(chainActive.Tip(), Params().GetConsensus()))
        {
            reason = "Community Fund has been enabled and you are using an old version of Skyrus, please update.";
            fObsolete = true;
        }
