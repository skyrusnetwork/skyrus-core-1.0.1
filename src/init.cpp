...
// Copyright (c) 2017 The Skyrus developers
 ...
 * between skyrusd, and skyrus-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), SKYRUS_CONF_FILENAME));
    if (mode == HMM_SKYRUSD)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SKYRUS_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-addanonserver=<ip>", _("Add a SkxTech node to use for private transactions"));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/skyrusdev/skyrus-core>";
    const std::string URL_WEBSITE = "<https://skyrus.org>";
...
/** Sanity checks
 *  Ensure that Skyrus is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Skyrus process is using the data directory.
    ...
    if(GetBoolArg("-staking", true))
        threadGroup.create_thread(boost::bind(&SkyrusStaker, boost::cref(chainparams)));
#endif
