        ...
        if(pcfundindex->GetPaymentRequestIndex(vec))
        {
            BOOST_FOREACH(const CFund::CPaymentRequest& prequest, vec) {
                ...
                if(prequest.fState == CFund::ACCEPTED && prequest.paymenthash == uint256() &&
                    ...
                    if(CFund::FindProposal(prequest.proposalhash, proposal)) {
                        coinbaseTx.vout.resize(coinbaseTx.vout.size()+1);
                        CSkyrusAddress addr(proposal.Address);
                        if (!addr.IsValid())
                            continue;
...
void SkyrusStaker(const CChainParams& chainparams)
{
    LogPrintf("SkyrusStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("skyrus-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in SkyrusStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running SkyrusStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SkyrusStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SkyrusStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("SkyrusStaker: ProcessNewBlock, block not accepted");
        }
    }
...
{
    LogPrintf("skyrusMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("skyrus-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("polisMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
