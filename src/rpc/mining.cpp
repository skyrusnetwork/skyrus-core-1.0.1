...
UniValue generatetoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. numblocks    (numeric, required) How many blocks are generated immediately.\n"
            "2. address    (string, required) The address to send the newly generated skyrus to.\n"
...
// NOTE: Unlike wallet RPC (which use SKX values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/skyrus/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/skyrus/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Skyrus is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Skyrus is downloading blocks...");
...
UniValue submitblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "See https://en.skyrus.it/wiki/BIP_0022 for full specification.\n"
...
UniValue estimatesmartfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in SKX)\n"
